...
mod tests {
    ...
    #[test]
    fn civil_negative_days_roundtrip() {
        // A date far before 1970 exercises the negative `z`/`y` branches of the
        // branchless Gregorian arithmetic in `civil_from_days`/`days_from_civil`.
        let (y, m, d) = civil_from_days(-1_000_000);
        assert_eq!(days_from_civil(y, m, d), -1_000_000);
    }
